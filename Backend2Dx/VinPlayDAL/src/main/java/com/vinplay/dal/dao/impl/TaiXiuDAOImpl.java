...
implements TaiXiuDAO {
    ...
    public List<ResultTaiXiu> getLichSuPhien(int number, int moneyType) throws SQLException {
        ...
        try (Connection conn = ConnectionPool.getInstance().getConnection("mysqlpool_minigame");){
            String sql = "SELECT * FROM result_tai_xiu WHERE money_type=" + moneyType + " ORDER BY `timestamp` DESC LIMIT 0," + number;
            ...
            while (rs.next()) {
                ...
                entry.timestamp = CommonUtils.convertTimestampToString((java.util.Date)timestamp, CommonUtils.FORMAT_DATE_01);
                results.add(0, entry);
            }
            rs.close();
            stmt.close();
    ...
    public List<TransactionTaiXiu> getLichSuGiaoDich(String nickname, int number, int moneyType) throws SQLException {
        ...
        try {
            call = conn.prepareCall("CALL tx_get_lich_su_giao_dich(?,?,?)");
            int param = 1;
            ...
            while (rs.next()) {
                ...
                entry.timestamp = CommonUtils.convertTimestampToString((java.util.Date)date, CommonUtils.FORMAT_DATE_01);
    ...
    public List<TransactionTaiXiuDetail> getChiTietPhien(long referenceId, int moneyType) throws SQLException {
        ...
        CallableStatement call = null;
        try {
            call = conn.prepareCall("CALL tx_get_chi_tiet_phien(?,?)");
            int param = 1;
    ...
    public ResultTaiXiu getKetQuaPhien(long referenceId, int moneyType) throws SQLException {
        ...
        try (Connection conn = ConnectionPool.getInstance().getConnection("mysqlpool_minigame");){
            String sql = "SELECT * FROM result_tai_xiu WHERE reference_id=" + referenceId + " AND money_type=" + moneyType;
            PreparedStatement stmt = conn.prepareStatement(sql);
            ResultSet rs = stmt.executeQuery();
            if (rs.next()) {
    ...
    public int getSoLanRutLoc(String username) throws SQLException {
        int soLanRut = 0;
        try (Connection conn = ConnectionPool.getInstance().getConnection("mysqlpool_minigame");){
            String sql = "SELECT so_lan_rut FROM user_rut_loc WHERE user_name='" + username + "'";
            PreparedStatement stmt = conn.prepareStatement(sql);
            ResultSet rs = stmt.executeQuery();
            if (rs.next()) {
    ...
    public ReportMoneySystemModel getReportTXToDay() {
        ...
        for (Map.Entry<String, ReportModel> entry : reportMap.entrySet()) {
            if (!((String)entry.getKey()).contains(today) || !((String)entry.getKey()).contains("TaiXiu")) continue;
