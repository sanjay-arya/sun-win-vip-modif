...
import game.modules.chat.cmd.sendworld.ChatInfoMsg;
import game.modules.chat.cmd.sendworld.ChatMsg;
...
//        BitZeroServer.getInstance().getTaskScheduler().scheduleAtFixedRate(this.ChatBot, 10,8, TimeUnit.SECONDS);
    ...
    public void handleClientRequest(User user, DataCmd dataCmd) {
        switch (dataCmd.getId()) {
            case MiniGameCMD.CHATWORLD_SUBSCRIBE: {
                this.subscribe(user);
                break;
            }
            case MiniGameCMD.CHATWORLD_UNSUBSCRIBE: {
                this.unsubscribe(user);
                break;
            }
            case MiniGameCMD.CHATWORLD: {
    ...
    private void chat(User user, DataCmd dataCmd) {
        ...
        if (this.allowUserChat(user.getName(), daiLy) && !this.containBadword(username, cmd.message)) {
            if (daiLy == 100) {
                ...
                long currentMoney = this.userService.getCurrentMoneyUserCache(username, "vin");
                if(currentMoney<200){
                    ...
                    MoneyResponse res = this.userService.updateMoney(username, -200, "vin", "ChatWorld", "Tru tien ChatWorld", "Noi dung:" + cmd.message, 0L, 0L, TransType.END_TRANS);
                    if (!res.isSuccess()) {
                        ChatMsg msg = new ChatMsg();
                        msg.Error = 6;
                        this.send((BaseMsg)msg, user);
    ...
    private final class ChatBot
        ...
        public void run() {
            try {
                ChatWorldModule.this.chatBot(GetRandomBot(),GetRandomMessageChat());
            }
            catch (Exception e) {
                e.printStackTrace();
            }
