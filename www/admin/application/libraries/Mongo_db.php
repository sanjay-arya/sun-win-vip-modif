...
Class Mongo_db{
    ...
    private function prepare()
    {
        ...
        if(isset($this->config[$this->activate]) == TRUE)
        {
            ...
            if(empty($this->config[$this->activate]['db_auth']))
            {
                show_error("db_auth name missing from mongodb config group : {$this->activate}", 500);
            }
            else
            {
                $this->db_auth = trim($this->config[$this->activate]['db_auth']);
            }
    ...
    private function connect()
    {
        ...
        try
        {
            $dns = "mongodb://{$this->hostname}:{$this->port}/{$this->db_auth}";
