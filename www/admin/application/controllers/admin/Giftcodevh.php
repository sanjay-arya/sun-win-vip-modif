...
Class Giftcodevh extends MY_Controller
{
    ...
    function vhaddajax()
    {
        ...
        curl_setopt($ch, CURLOPT_POSTFIELDS,'c=306&gp=' . $money . '&gq=' . $quantity
            . '&gl=' . $version . '&mt=' . $moneytype . '&gs=GVH&type=3');
            ...
            if ($data->success == true) {
                if ($data->errorCode == 0) {
                    if ($moneytype == 1) {
                        ...
                        $this->logadmin_model->create($this->logadmingiftcode(17, "", $admin_info->UserName, $quantity, $money * 1000000, "Coin"));
                    }
                    echo json_encode("1");
                }
            } else {
    ...
    function vhaddgcajax()
    {
        ...
        curl_setopt($ch, CURLOPT_POSTFIELDS,'c=301&gp=' . $money . '&gq=' . $quantity
            . '&gs=' . $source . '&gl=' . $version . '&mt=' . $moneytype . '&type=3');
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch,CURLOPT_TIMEOUT,3600);
        $server_output = curl_exec ($ch);
        $data = json_decode($server_output);
            ...
            if ($data->success == true) {
                if ($data->errorCode == 0) {
                    if ($moneytype == 1) {
                        ...
                        $this->logadmin_model->create($this->logadmingiftcode(18, $nickname, $admin_info->UserName, $quantity, $money * 1000000, "Coin"));
                    }
                    echo json_encode("1");
                }
            } else {
