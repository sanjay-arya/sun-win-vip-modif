...
function trove(){
	return '<a rel="nofollow"  href="'.$_SERVER['HTTP_REFERER'].'"><< Spin lại</a>';
}
function tieude($n){
...
echo $head;
	...
	if(isset($email)){
		...
		echo noidung("Nhập sai E-Mail hoặc mã xác nhận rồi bạn! Hoặc chưa hết Time bị chặn!");
			setcookie("ban",time()+$setthoigianban,$setthoigianban);$_SESSION['ban']=time()+$setthoigianban;
	...
	//echo $tght;
		...
		if(!empty($cookban)){ 
				cbjs("Bạn bị khóa $ctgcl vì nhập sai ... vui lòng chờ hết Time trên để thử lại ...").chmt($tgcl,'index.php?qmk&');
...
//Time chặn một truy cập nguy hiểm
