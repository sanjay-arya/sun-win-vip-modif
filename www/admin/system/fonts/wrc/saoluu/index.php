...
function display_select($sth,$q){
 ...
 if ($is_sht || $is_shd) {$w='wa';
   ...
   $sqldr.="<div class='dot'>
&nbsp;MySQL Server:
&nbsp;&#183;<a href='$url&q=show+variables'>Xem thiết lập hệ thống</a>
&nbsp;&#183;<a href='$url&q=show+status'>Xem thống kê</a>
&nbsp;&#183;<a href='$url&q=show+processlist'>Tác vụ</a>";
   if ($is_shd) $sqldr.="&nbsp;&#183;Tạo CSDL mới: <input type='text' name='new_db' placeholder='type db name here'> <input type='submit' name='crdb' value='Create'>";
   $sqldr.="<br>";
   if ($is_sht) $sqldr.="&nbsp;Database:&nbsp;&#183;<a href='$url&q=show+table+status'>Xem trạng thái bảng</a>";
   $sqldr.="</div>";
 }
 if ($is_sht){
...
<!DOCTYPE html>
<html>
<head><title>Quản lí CSDL mini</title>
<meta charset="utf-8">
<style type="text/css">
body{font-family:Arial,sans-serif;font-size:80%;padding:0;margin:0;margin:auto;max-width:700px}
...
<div class="inv">
<a href="http://phpminiadmin.sourceforge.net/" target="_blank"><b>PMA 1.9</b></a>
<?php if ($_SESSION['is_logged'] && $dbh){ ?>
 | <a href="?<?php eo($xurl)?>&q=show+databases">CSDL</a>: <select name="db" onChange="frefresh()"><option value='*'> - chọn/cập nhật -</option><option value=''> - coi hết -</option>
<?php echo get_db_select($dbn)?></select>
<?php if($dbn){ $z=" &#183; <a href='".hs($self."?$xurl&db=$dbn"); ?>
<?php echo $z.'&q='.urlencode($SHOW_T)?>'>Xem bảng</a>
<?php echo $z?>&shex=1'>Xuất khẩu</a>
<?php echo $z?>&shim=1'>Nhập khẩu</a>
<?php } ?>
 | <a href="?showcfg=1">Cài đặt</a>
<?php } ?>
<?php if ($GLOBALS['ACCESS_PWD']){?> | <a href="?<?php eo($xurl)?>&logoff=1" onclick="logoff()">Thoát</a> <?php }?>
...
Nhập lệnh mysql (hoặc nội dung .sql ";"):&nbsp;<button type="button" class="qnav" onclick="q_prev()">&lt;</button><button type="button" class="qnav" onclick="q_next()">&gt;</button><br>
<textarea id="q" name="q" ><?php eo($SQLq)?></textarea><br>
<input type="submit" name="GoSQL" value="Thực thi" onclick="return chksql()" style="width:100px">&nbsp;&nbsp;
<input type="button" name="Clear" value="Erase hết " onclick="document.DF.q.value=''" style="width:100px">
</div>

<div class="dot" style="padding:5px 0 5px 20px">
Giá trị: <b><?php eo($reccount)?></b> in <b><?php eo($time_all)?></b> sec<br>
...
?>
<center>
<h3>Kết nội được bảo vệ bằng mật khẩu:</h3>
<div style="width:400px;border:1px solid #999999;background-color:#eeeeee">
Mật khẩu: <input type="password" name="pwd" value="">
...
?>
<center>
<h3>DB thiết lập kết nối</h3>
<div class="frm">
<label class="l">Account:</label><input type="text" name="v[user]" value="<?php eo($DB['user'])?>"><br>
<label class="l">Mật khẩu:</label><input type="password" name="v[pwd]" value=""><br>
<div style="text-align:right"><a href="#" class="ajax" onclick="cfg_toggle()">Tùy chọn nâng cao</a></div>
...
<input type="submit" value="áp dụng "><input type="button" value="Cancel" onclick="window.location='<?php eo($self)?>'">
</center>
</div>
</center>
<?php
...
function db_connect($nodie=0){
 ...
 if (!$dbh) {
    $err_msg='Không thể kết nối tới vì: '.mysql_error();
 ...
 if ($dbh && $DB['db']) {
  ...
  if (!$res) {
     $err_msg='Select không dfược CSDL: '.mysql_error();
     if (!$nodie) die($err_msg);
  }else{
...
<div>
<?php if ($t && !strpos($t,',')){?>
 <label><input type="radio" name="et" value="csv"> .csv (Excel kiểu, chỉ dử liệu và 1 bảng)</label>
<?php }else{?>
<label>&nbsp;( ) .csv</label> <small>(to export as csv - go to 'show tables' and export just ONE table)</small>
<?php }?>
</div>
<br>
<div><label><input type="checkbox" name="gz" value="1"> nén lại .gz</label></div>
<br>
...
?>
<center>
<h3>Nhập khẩu CSDL</h3>
<div class="frm">
<b>.sql</b> or <b>.gz</b> file: <input type="file" name="file1" value="" size=40><br>
<input type="hidden" name="doim" value="1">
<input type="submit" value="Tải và nhập ngay" onclick="return ays()"><input type="button" value=" Cancel " onclick="window.location='<?php eo($self.'?'.$xurl.'&db='.$DB['db'])?>'">
</div>
<br><br><br>
<!--
...
function do_import(){
 ...
 if ($F && $F['name']){
  ...
  if ($pi['extension']!='sql'){//if not sql - assume .gz
     ...
     }else{$err_msg='Lỗi khi giải nén gz';}
  }
  if (!$err_msg){
