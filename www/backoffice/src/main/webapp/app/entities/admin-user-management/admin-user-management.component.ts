...
export class AdminUserManagementComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.userListSubscription = this.eventManager.subscribe('adminUserListModification', () => this.loadAll());
  ...
  deleteUser(user: User): void {
    const modalRef = this.modalService.open(AdminUserManagementDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  onSubmit(filter: { id: string; loginName: string }): void {
    this.userService
      .queryFillrole({
        page: this.page - 1,
        size: this.itemsPerPage,
