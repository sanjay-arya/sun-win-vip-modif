...
<?php else: ?>
    <div class="wrapper">
        ...
        <div class="widget">
            ...
            <div class="title">
                <h6>Coincard reward rate</h6>
            </div>

                <input type="button" id="updateall" class="blueB logMeIn" value="Update All" style="margin-right: 370px">
            <div class="formRow">
            </div>
...
<script>    function updateratio(nickname, ratio) {
        if (!confirm('Are you sure you want to update the coincard reward rate??')) {
            return false;
        }
        $("#spinner").show();
        $.ajax({
            ...
            success: function (result) {
                ...
                if (result.errorCode == 0) {
                    ...
                    $("#statuspenđing").html("You have successfully updated the coincard reward rate");

                } else if (result.errorCode == 1044) {
                    $("#bsModal3").modal("show");
                    $("#statuspenđing").css({"color": "red"});
                    $("#statuspenđing").html("The registration % is incorrect (the required registration % must be between 10 - 100");
                } else if (result.errorCode == 1045) {
                    $("#bsModal3").modal("show");
                    $("#statuspenđing").css({"color": "red"});
                    $("#statuspenđing").html("Level 1 agent nickname is not in the database");
                }
                else if (result.errorCode == 1046) {
                    $("#bsModal3").modal("show");
                    $("#statuspenđing").css({"color": "red"});
                    $("#statuspenđing").html("The list of nick names and the list of percent sent to the server do not match");
                }            }, error: function () {
                $("#spinner").hide();
                $("#resultsearch").html("System overload. Please call 19008698 or F5 to return to the pages");
            },timeout : 40000
    ...
    function resultSearchTransction(stt, nickname, ratio) {
        ...
        rs += "<td class='rate'>" + "<input type='button' id='updateratio' value='Update rate' class='button blueB' style='margin-left: 70px'>" + "</td>";
        rs += "</tr>";
        return rs;
    }</script>
