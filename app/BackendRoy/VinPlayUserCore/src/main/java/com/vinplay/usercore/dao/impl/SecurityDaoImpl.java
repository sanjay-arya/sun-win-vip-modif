...
implements SecurityDao {
	...
	@Override
	public boolean updateUserInfos(int userId, String identification, String email, String birthday, int gender,
			String address, boolean status, String referralCode,String mobile) throws SQLException {
		boolean res = false;
		Debug.trace("update_user_info4");
        try (Connection conn = ConnectionPool.getInstance().getConnection("mysqlpoolname");){
            String sql = "UPDATE users SET identification=?, email=?, birthday=?, gender=?, address=?, status=? , referral_code=? , mobile=? WHERE id=?";
            PreparedStatement stm = conn.prepareStatement(sql);
            ...
            stm.setInt(8, userId);
            stm.setString(9, mobile);
            if (stm.executeUpdate() == 1) {
                res = true;
            }
            Debug.trace("update_user_info5");
            stm.close();
        }
        return res;
    }
