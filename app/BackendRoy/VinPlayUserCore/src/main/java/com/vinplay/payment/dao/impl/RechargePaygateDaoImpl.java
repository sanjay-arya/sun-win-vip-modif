...
//		}
	...
	public boolean updatePendingStatusToFailedAfterMinus(int minutes, String provider) {
		try {
			...
			String dateTimeAfterMinus = DateTimeUtils.getTimeSpace(currenTime, "yyyy-MM-dd HH:mm:ss", minutes * (-1),
					"mm");
			...
			db.getCollection(DEPOSIT_COLLECTION).updateMany(conditions, new Document("$set", updateFields));
			return true;
		} catch (Exception e) {
			logger.error(e);
			return false;
	...
	public Boolean Update(DepositPaygateModel depositPayWellModel) {
		try {
			MongoDatabase db = MongoDBConnectionFactory.getDB();
			BasicDBObject updateFields = new BasicDBObject();
			updateFields.append("CreatedAt", depositPayWellModel.CreatedAt);
			updateFields.append("ModifiedAt", VinPlayUtils.getCurrentDateTime());
			updateFields.append("IsDeleted", false);
			updateFields.append("CartId", depositPayWellModel.Id);
			updateFields.append("ReferenceId", depositPayWellModel.Id);
			updateFields.append("UserId", depositPayWellModel.UserId);
			updateFields.append("Username", depositPayWellModel.Username);
			updateFields.append("Nickname", depositPayWellModel.Nickname);
			updateFields.append("RequestTime", depositPayWellModel.RequestTime);
			updateFields.append("BankCode", depositPayWellModel.BankCode);
			updateFields.append("MerchantCode", depositPayWellModel.MerchantCode);
			updateFields.append("PaymentType", depositPayWellModel.getPayTypeStr());
			updateFields.append("ProviderName", depositPayWellModel.ProviderName);
			updateFields.append("Amount", depositPayWellModel.Amount);
			updateFields.append("AmountFee", depositPayWellModel.AmountFee);
			updateFields.append("Status", depositPayWellModel.Status);
			updateFields.append("BankAccountNumber", depositPayWellModel.BankAccountNumber);
			updateFields.append("BankAccountName", depositPayWellModel.BankAccountName);
			updateFields.append("Description", depositPayWellModel.Description);
			updateFields.append("UserApprove", depositPayWellModel.UserApprove);
			updateFields.append("AgentBankCode", depositPayWellModel.AgentBankCode);
			updateFields.append("AgentBankAccountName", depositPayWellModel.AgentBankAccountName);
			updateFields.append("AgentBankAccountNumber", depositPayWellModel.AgentBankAccountNumber);
			updateFields.append("Content", depositPayWellModel.Content);
	...
	public Boolean UpdateStatusById(String id, int status, String userApprove) {
		try {
			...
			updateFields.append("ModifiedAt", VinPlayUtils.getCurrentDateTime());
			updateFields.append("UserApprove", userApprove);
			db.getCollection(DEPOSIT_COLLECTION).updateOne(new Document("Id", id),
					new Document("$set", updateFields));
			return true;
		} catch (Exception e) {
			return false;
	...
	public DepositPaygateModel GetById(String Id) {
		try {
			...
			DepositPaygateModel model = new DepositPaygateModel(document.getString("Id"),
					...
					document.getString("AgentBankCode"), document.getString("AgentBankAccountName"),
					document.getString("AgentBankAccountNumber"), document.getString("Content"));
			return model;
		} catch (Exception e) {
			logger.error(e);
			return null;
...
//					.into(new ArrayList<>());
	...
	public ArrayList<Object> find(HashMap<String, Object> conditions, int page, int maxItem) throws Exception {
		try {
			...
			return db.getCollection(DEPOSIT_COLLECTION).find((Bson) new Document(conditions))
