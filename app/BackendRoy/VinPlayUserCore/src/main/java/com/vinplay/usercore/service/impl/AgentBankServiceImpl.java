	...
	private String ValidateConfig(AgentBank agentBank) {
		try {
			...
			if (agentBank.getId() == 0) { // Create
				AgentBank agentBankCheck = dao.getByBankNumber(agentBank.getBank_number());
				if (agentBankCheck != null && agentBank.getBank_code().equals(agentBankCheck.getBank_code())) {
						return "Bank number is exist";
				}
				
				agentBankCheck = dao.getByBankCode(agentBank.getAgent_code(), agentBank.getBank_code());
				if (agentBankCheck != null && agentBank.getBank_number().equals(agentBankCheck.getBank_number()))
				...
				if (oldAgentBank != null && oldAgentBank.getId() != agentBank.getId()
						&& agentBank.getBank_number().equals(oldAgentBank.getBank_number())
						&& agentBank.getBank_code().equals(oldAgentBank.getBank_code()))
					return "Bank number or code is exist";
			}
