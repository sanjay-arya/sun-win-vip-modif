...
public class RejectAgentTransactionByBOProccessor implements BaseProcessor<HttpServletRequest, String> {
    @Override
    public String execute(Param<HttpServletRequest> param) {
        ...
        if(StringUtils.isBlank(description))
        	description = "Rejected by: " + userApproved + " at: " + new Date();
        ...
        try {
        	String rs = service.updateStatus(transId, PAYSTATUS.FAILED.getId(), description, userApproved);
        	if("success".equalsIgnoreCase(rs)) {
        		String message = "<b>[BACK-OFFICE][REJECTED] Yêu cầu nạp tiền từ đại lý: " + agentModel.getUsername() + "</b>";
        		message += "\n Mã yêu cầu: <b>" + transId + " (" + transaction.CreatedAt.toString() + ")" + "</b>";
	            message += "\n Số tiền: <b>"+ transaction.Money + "</b>";
	            message += "\n Người thực hiện: <b>" + userApproved + "</b>";
