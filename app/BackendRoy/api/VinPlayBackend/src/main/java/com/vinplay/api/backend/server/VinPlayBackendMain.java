	...
	public static void main(String[] args) {
		try {
			...
			try {
				...
				e.printStackTrace();
				logger.info(("ERRROR INIT loadCommands " + e.getMessage()));
			}
			...
			try {
				...
				e.printStackTrace();
				logger.info(("ERRROR INIT HazelcastLoader " + e.getMessage()));
			}
			...
			try {
				...
				e.printStackTrace();
				logger.info(("ERRROR INIT MongoDBConnectionFactory " + e.getMessage()));
			}
			...
			try {
				...
				e.printStackTrace();
				logger.info(("ERRROR INIT RMQApi " + e.getMessage()));
			}
			...
			try {
				...
				e.printStackTrace();
				logger.info(("ERRROR INIT GameCommon " + e.getMessage()));
			}
...
//			server.setHandler(collection);
	...
	public static class APIAgentServlet extends HttpServlet {
		...
		private void onExecute(HttpServletRequest request, HttpServletResponse response) throws IOException {
			...
			if (requestMap.containsKey("c")) {
				String command = request.getParameter("c");
				List<String> commands = Arrays.asList("9424", "9425", "9426", "9427", "9428", "9431", "8840", "72",
						"73", "74", "75", "9440", "9441", "9442", "9443", "9444", "9445", "9446", "9447", "9448",
				...
				param.set(request);
				try {
					String responseString = (String) controller
							.processCommand(Integer.valueOf(Integer.parseInt(command)), param);
					response.getWriter().println(responseString);
				} catch (NoCommandRegistered e2) {
					logger.debug("COMMAND NOT FOUND");
					response.getWriter().println("COMMAND NOT FOUND");
	...
	public static class VerifyMobileViaTelegramServlet extends HttpServlet {
		...
		private String onExecute(HttpServletRequest request, HttpServletResponse response) throws IOException {
			...
			try {
				...
				e.printStackTrace();
				logger.error("Verify via telegram: " + e.getMessage());
				return new BaseResponseModel<String>(false, "1001", "Error system. Please contact admin to support")
						.toJson();
			}
			...
			try {
				...
				return new BaseResponseModel<String>(false, "1001", "Not found chat session on device of customer")
						.toJson();
			}
				...
				try {
					...
					if (!userMap.containsKey(nickname)) {
						code = postRequest(URLAPITELEGRAM, telegramId,
								"Your trading session is expried, please reload page and login for verify again.");
						return code == 200 ? new BaseResponseModel<String>(true, "0", "success").toJson()
								: new BaseResponseModel<String>(false, "1001", "Thông tin xác thực không đúng")
										.toJson();
					}
					...
					if (!userCache.getAccessToken().equals(accessToken)) {
						code = postRequest(URLAPITELEGRAM, telegramId,
								"Your trading session is expried, please reload page and login for verify again.");
						return code == 200 ? new BaseResponseModel<String>(true, "0", "success").toJson()
								: new BaseResponseModel<String>(false, "1001", "Information is invalid").toJson();
					}
			...
			String phoneNumber = "";
			try {
				phoneNumber = jsonObject.getAsJsonObject("message").getAsJsonObject("contact").get("phone_number")
						.getAsString();
			} catch (Exception e) {
			}
			...
			if (phoneNumber == null || phoneNumber.trim().equals("")) {
				String body = "{\"chat_id\":" + telegramId
						+ ",\"text\":\"Please press send telephone number button for verify on GO88vin\",\"reply_markup\":{\"keyboard\":[[{\"text\":\"Send telephone number\",\"request_contact\":true}]],\"one_time_keyboard\":true,\"resize_keyboard\":true}}";
				...
				if (checked) {
					code = postRequest(URLAPITELEGRAM, telegramId, "Your telephone number is verified");
					return code == 200 ? new BaseResponseModel<String>(true, "0", "success").toJson()
							: new BaseResponseModel<String>(false, "1001", "Your telephone number is verified")
									.toJson();
				}
				...
				if (checked) {
					code = postRequest(URLAPITELEGRAM, telegramId, "Your telephone number is verified success");
					String nickName = "";
					try {
					...
					return code == 200 ? new BaseResponseModel<String>(true, "0", "success").toJson()
							: new BaseResponseModel<String>(false, "1001", "Your telephone number is verified success")
									.toJson();
				} else {
					code = postRequest(URLAPITELEGRAM, telegramId,
							"Verify telephone number failure, please contact customer care support for help.");
