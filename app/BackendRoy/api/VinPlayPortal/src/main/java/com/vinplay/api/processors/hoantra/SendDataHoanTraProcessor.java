...
public class SendDataHoanTraProcessor implements BaseProcessor<HttpServletRequest, String> {
    ...
    public String execute(Param<HttpServletRequest> param) {
		...
		if (!isToken) {
			res.setData("Your trading session is expired, please reload page and login again.");
        ...
        try {
            ...
            if (new ReturnMoney().countListHoanTraHistories(date, nickname) > 0) {
            	...
            	res.setData("Your have received fund on: " + date.toString());
    			return res.toJson();
            }
    ...
    private String sendHoanTra(Date date, String nickname) throws SQLException {
		...
		if (count > 0) {
			String[] data = new String[] {
					"Your have made the fund: " + count + " before. Please check again",
					"Fund failure: " + 0, "At time: " + (date == null ? "all" : date.toLocalDate()) };
			return BaseResponse.success(data, count);
		}
		
        int[] countSendHoanTra = new ReturnMoney().addMoneyHoanTra(date, nickname);
        String [] data = new String[]{
                "Fund success: " + countSendHoanTra[0],
                "Fund failure: " + countSendHoanTra[1],
                "At time: " + (date == null ? "all" : date.toLocalDate())
        };
