...
public class InsertOrUpdateBankProcessor implements BaseProcessor<HttpServletRequest, String>{
	...
	public String execute(Param<HttpServletRequest> param) {
		...
		if(!isExist) {
			return BaseResponse.error(Constant.ERROR_BANK_NAME, "We don't support this bank, please contact customer care support for help.");
		}
		...
		try {
			...
			if(branch.length() >20) {
				return BaseResponse.error(Constant.ERROR_PARAM, "Maxleng of bank branch is 20 characters.");
			}
			if ("0".equals(type)) {
				// insert
