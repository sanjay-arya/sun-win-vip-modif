        ...
        public BanCaService()
        {
            ...
            Post("/bancaapi/smscallback", async parameters =>
            {
                try
                {
                    ...
                    var contentCode = jdata["contentCode"].Value; // Name game, nội dung ddược đăng ký trên cổng thanh toán
                    ...
                    var mode = jdata["mode"].Value; // Type giao dịch gồm 2 loại: -check: giao dịch kiểm tra xem tài khoản useringame có tồn tại không(nếu có mới có giao dịch real); - real: Transaction ghi sản lượng thật, đã trừ tiền người dùng
            ...
            Get("/bancaapi/muacardcallback", async parameters =>
            {
                try
                {
                    ...
                    int status = this.Request.Query["status"]; // status: Status thẻ (3: Nạp thành công, 4: Thẻ sai, 5: Thẻ bị thu hồi do sai mệnh giá)
