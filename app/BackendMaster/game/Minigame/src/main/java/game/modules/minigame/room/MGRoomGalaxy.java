...
public class MGRoomGalaxy extends MGRoom {
    public long pot;
    public long fund;
    ...
    private final Runnable gameLoopTask = new MGRoomGalaxy.GameLoopTask();
    ...
    public void autoPlay(User user, String lines) {
        ...
        Map<String, AutoUserGalaxy> map2 = map = this.usersAuto;
        synchronized (map2) {
        ...
        if (lineArr.length > 0 && !linesStr.isEmpty()) {
            if (totalBetValue > 0L) {
                if (totalBetValue <= currentMoney) {
                    MoneyResponse moneyRes = this.userService.updateMoney(username, -totalBetValue, this.moneyTypeStr, Games.GALAXY.getName(),
                            this.gameID,
...
//                                GameUtils.sendSMSToUser(username, "Chuc mung " + username + " da no hu game Kim Cuong phong " + this.betValue + ". So tien no hu: " + totalPrizes + " Vin");
                        ...
                        try {
                            if (!u.isBot()) {
                                this.pgService.logGalaxy(refernceId, username, this.betValue, linesStr, linesWin, prizesOnLine, result, totalPrizes, this.moneyType, currentTimeStr);
