...
import game.modules.slot.cmd.send.bikini.*;
...
public class BikiniRoom extends SlotRoom {
    ...
    public BikiniRoom(BikiniModule module, byte id, String name, short moneyType, long pot, long fund, int betValue,
        ...
        this.gameID = Games.BIKINI.getId() + "";
    ...
    public void forceStopAutoPlay(User user) {
        ...
        synchronized (map) {
            ...
            ForceStopAutoPlayBikiniMsg msg = new ForceStopAutoPlayBikiniMsg();
            SlotUtils.sendMessageToUser((BaseMsg) msg, user);
        }
    }

    public ResultBikiniMsg play(String username, String linesStr) {
        ...
        try {
            ...
            Debug.trace("error get free spin Bentley");
        }
...
//        Debug.trace("addMoneyToPot  " + money, "pot  " + this.pot);
    ...
    private void broadcastBigWin(String username, byte result, long totalPrizes) {
        BigWinBikiniMsg bigWinMsg = new BigWinBikiniMsg();
    ...
    public ResultBikiniMsg playNormal(String username, String linesStr, long referenceId) {
        ...
        ResultBikiniMsg msg = new ResultBikiniMsg();
            ...
            if (totalBetValue > 0L) {
                if (totalBetValue <= moneyAvailable) {
                    ...
                    if (!u.isBot()) {
                        ...
                        DailyQuestUtils.playerPlayGame(username,Games.BIKINI.getId(),totalBetValue);
...
//                        long moneyToFund = totalBetValue - fee - moneyToPot - moneyToFundJackpot - moneyToFundMinigame;
                        ...
                        totalPrizes += (long) slot11IconWildLienTucTableInfo.money * this.betValue;
...
//                                    this.fundJackPot -= this.initPotValue;
                        ...
                        if (totalPrizes != 0 && !u.isBot()) {
                            if ((moneyRes = this.userService.updateMoney(username, totalPrizes, this.moneyTypeStr, this.gameName,
                                ...
                                if (this.moneyType == 1 && moneyExchange >= (long) BroadcastMessageServiceImpl.MIN_MONEY) {
                                    this.broadcastMsgService.putMessage(Games.BIKINI.getId(), username, moneyExchange - totalBetValue);
                                }
                            }
                        }
                        ...
                        try {
                            if (!u.isBot()) {
                                this.slotService.logBikini(referenceId, username, this.betValue, linesStr, linesWin, prizesOnLine, result, totalPrizes, currentTimeStr);
...
//        UserCacheModel u = this.userService.getUser(username);
//        long totalBetValue = lineArr.length * this.betValue;
//        ResultBikiniMsg msg = new ResultBBikiniMsg();
//
...
//                                        this.broadcastMsgService.putMessage(Games.BIKINI.getId(), username, moneyExchange - (long) this.betValue);
//                                    }
//                                }
//                            }
...
//                            try {
//                                if(!u.isBot()){
//                                    this.slotService.logBikini(referenceId, username, (long)this.betValue, linesStr, linesWin, prizesOnLine, result, totalPrizes, currentTimeStr);
//
//                                }
//                                if (result == 3 || result == 4) {
...
//    }
    ...
    public ResultBikiniMsg playFreeDaily(String username) throws Exception {
        //long startTime = System.currentTimeMillis();
        // String currentTimeStr = DateTimeUtils.getCurrentTime();
    ...
    public ResultBikiniMsg playFree(String username, String linesStr, int ratio, long referenceId) {
        long startTime = System.currentTimeMillis();
        String currentTimeStr = DateTimeUtils.getCurrentTime();
        short result = 0;
        String[] lineArr = linesStr.split(",");
...
//                this.fundJackPot -= this.initPotValue;
    ...
    public short play(User user, String linesStr) throws Exception {
        ...
        ResultBikiniMsg msg = null;
        BikiniFreeDailyMsg freeDailyMsg = new BikiniFreeDailyMsg();
        ...
        if (this.isUserMinimize(user)) {
            MinimizeResultBikiniMsg miniMsg = new MinimizeResultBikiniMsg();
    ...
    private void savePot() {
        ...
        if (currentTime - this.lastTimeUpdatePotToRoom >= 3000L) {
            ...
            UpdatePotBikiniMsg msg = new UpdatePotBikiniMsg();
            msg.value = this.pot;
    ...
    protected void playListAuto(List<AutoUser> users) {
        for (AutoUser user : users) {
            try {
                ...
                Logger.getLogger(BikiniRoom.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
        users.clear();
    }
