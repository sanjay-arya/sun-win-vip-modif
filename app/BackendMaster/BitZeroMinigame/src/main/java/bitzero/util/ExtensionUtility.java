...
implements IBZEventListener {
    ...
    public User canLogin(UserInfo userInfo, String password, ISession session) {
        ...
        Debug.trace("Call login Minigame");
    ...
    public void sendLogoutOK(User user) {
        this.sendLogoutResponse(user, 0);
    }
    ...
    public void sendLoginResponse(User user, int err) {
        ...
        response.setId(Short.parseShort(SystemRequest.Login.getId().toString()));
        response.setTargetController(DefaultConstants.CORE_EXTENSIONS_CONTROLLER_ID);
        response.setContent(new byte[]{(byte)0});
        response.setRecipients(user.getSession());
        response.write();

        Debug.trace("send login sucess  ",user.getName());
    }

    public void sendLogoutResponse(User user, int err) {
        Response response = new Response();
        response.setId(Short.parseShort(SystemRequest.Logout.getId().toString()));
        response.setTargetController(DefaultConstants.CORE_EXTENSIONS_CONTROLLER_ID);
        response.setContent(new byte[]{(byte)err});
        response.setRecipients(user.getSession());
        response.write();
    ...
    public void send(BaseMsg bmsg, Collection<ISession> recipients) {
        bz.getAPIManager().getResApi().sendExtResponse(bmsg, recipients);
    }
