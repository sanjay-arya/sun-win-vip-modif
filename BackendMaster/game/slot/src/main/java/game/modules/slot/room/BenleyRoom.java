...
public class BenleyRoom
    ...
    public BenleyRoom(BenleyModule module, byte id, String name, short moneyType, long pot, long fund, int betValue,
        ...
        this.gameID = Games.BENLEY.getId() + "";
            ...
            if (totalBetValue > 0L) {
                if (totalBetValue <= moneyAvailable) {
                    ...
                    if (!u.isBot()) {
                        ...
                        DailyQuestUtils.playerPlayGame(username,Games.BENLEY.getId(),totalBetValue);
                        ...
                        if (totalPrizes != 0 && !u.isBot()) {
                            if ((moneyRes = this.userService.updateMoney(username, totalPrizes, this.moneyTypeStr, this.gameName,
                                ...
                                if (this.moneyType == 1 && moneyExchange >= (long) BroadcastMessageServiceImpl.MIN_MONEY) {
                                    this.broadcastMsgService.putMessage(Games.BENLEY.getId(), username, moneyExchange - totalBetValue);
                                }
                            }
                        }
...
//    }
    ...
    public ResultBenleyMsg playFreeDaily(String username) throws Exception {
        //long startTime = System.currentTimeMillis();
        // String currentTimeStr = DateTimeUtils.getCurrentTime();
    ...
    public ResultBenleyMsg playFree(String username, String linesStr, int ratio, long referenceId) {
        long startTime = System.currentTimeMillis();
        String currentTimeStr = DateTimeUtils.getCurrentTime();
        short result = 0;
        String[] lineArr = linesStr.split(",");
