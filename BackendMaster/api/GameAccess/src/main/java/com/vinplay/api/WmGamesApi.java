 ...
 *
 */
//@WebServlet(name = "WmGamesApi", description = "API for Wm games", urlPatterns = { "/api/wm" })
public class WmGamesApi extends HttpServlet {
	/**
	 * 
	 */
	private static final long serialVersionUID = -7651078352307234600L;
	private static final Logger logger = Logger.getLogger(WmGamesApi.class);
	private WmService service = new WmServiceImpl();
...
//		}
		...
		if (InitData.isWMDown()) {
			...
			rf = new ResultFormat(Constant.ERROR_MAINTAIN, "LiveCasino WM đang bảo trì");
		...
		try {
			...
			case "Transfer": {
				...
				rf = service.fundTransfer(nickName, direction, amount, ipAddress);
				break;
			}
			case "CheckBalance": {
				rf = service.checkBalance(nickName);
				break;
			}
			case "Login": {
