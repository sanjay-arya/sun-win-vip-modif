 ...
 *
 */
public class EbetGamesApi extends HttpServlet {
	/**
	 * 
	 */
	private static final long serialVersionUID = -7651078352307234600L;
	private static final Logger logger = Logger.getLogger(EbetGamesApi.class);
	private EbetService service = new EbetServiceImpl();
...
//		}
		...
		if (InitData.isEbetDown()) {
			logger.info("EBET maintained");
			rf = new ResultFormat(Constant.ERROR_MAINTAIN, "LiveCasino EBET đang bảo trì");
		...
		try {
			...
			logger.error("EBET exception ,type=" + type, ex);
		}
	}
}