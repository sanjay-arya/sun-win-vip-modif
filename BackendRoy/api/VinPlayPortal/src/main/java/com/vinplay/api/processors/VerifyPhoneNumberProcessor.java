...
public class VerifyPhoneNumberProcessor implements BaseProcessor<HttpServletRequest, String> {
	...
	public String execute(Param<HttpServletRequest> param) {
		...
		if (!isToken) {
			return BaseResponse.error(Constant.ERROR_SESSION, "Your trading session has expried, please reload the page and login again.");
		}
		try {
			if (userService.isXacThucSDT(nickName)) {
				return BaseResponse.error(Constant.ERROR_PARAM, "Your account is verified telephone");
			}
		} catch (Exception e2) {
			logger.error(e2);
			return BaseResponse.error(Constant.ERROR_PARAM, "Your account is verified telephone");
		...
		try {
			if (userService.isPhoneUsed(phone)) {
				...
				return BaseResponse.error(Constant.ERROR_PARAM, "This telephone number has used");
			}
