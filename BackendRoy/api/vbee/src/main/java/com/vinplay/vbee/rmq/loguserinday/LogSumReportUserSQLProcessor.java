...
public class LogSumReportUserSQLProcessor implements BaseProcessor<byte[], Boolean> {
    ...
    public Boolean execute(Param<byte[]> param) {
		...
		if (!message.isBot() && !Consts.GAME3RD.LIST_GAME.contains(message.getActionName())
				&& !Consts.ADMIN.equals(message.getServiceName())) {
			try {
    ...
    public void getDataUserCache(LogMoneyUserMessage message) {
        ...
        try {
            ...
            if (Consts.REAL_DEPOSIT_VIN.contains(message.getActionName())) {
				...
				} else if (serviceID == Games.DIEM_DANH.getId()) { // diem danh
					logReportModel.attendance = Math.abs(amount);
					logCountUserPlay.attendance++;
				} else {
					logger.info("report rabbitmq serviceID =" + serviceID);
				}
            }
            ...
            if (writeNewDataCount) { // tao row moi
                ...
                boolean update = LogCountReportUtil.updateLogSQL(logCountUserPlay);
                if (!update) {
                    LogCountReportUtil.insertNewLogSQL(logCountUserPlay);
                }
            }
