...
public class UpdateMoneyProcessor implements BaseProcessor<byte[], Boolean> {
	...
	public Boolean execute(Param<byte[]> param) {
		...
		try {
			...
			if (Consts.RECHARGE_BY_BANK.equalsIgnoreCase(message.getActionName())
					|| (Consts.ADMIN.equalsIgnoreCase(message.getActionName()) && message.getMoneyExchange() > 0)) {
				userService.changeUserValue(message.getNickname(), message.getMoneyExchange(), false);
			}
			if (Consts.REQUEST_CASHOUT.equals(message.getActionName())
					|| (Consts.ADMIN.equalsIgnoreCase(message.getActionName()) && message.getMoneyExchange() < 0)) {
