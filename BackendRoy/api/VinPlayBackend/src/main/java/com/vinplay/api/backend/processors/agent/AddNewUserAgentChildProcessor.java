...
//		} catch (NumberFormatException e) { active = 1;}
		...
		if(userModel != null)
			return BaseResponse.error("-1", "Username is exist !");
		...
		if(userModel != null)
			return BaseResponse.error("-1", "Nickname is exist !");
		...
		userAgentModel.setCreatetime(Date.from(LocalDateTime.now().atZone(ZoneId.systemDefault())
	    	      .toInstant()));
