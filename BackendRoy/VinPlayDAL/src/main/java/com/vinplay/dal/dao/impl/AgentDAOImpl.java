...
public class AgentDAOImpl implements AgentDAO {
			...
			try {
				...
				if (rs.next()) {
					if (rs.getInt("active") == 0) {
						...
						map.put("mess", "Account is blocked");
						return map;
					}
	...
	public String changePasswordUserAgent(String nickname, String old_password, String new_password)
		...
		try (Connection conn = ConnectionPool.getInstance().getConnection(ConnectionPool.ADMIN_POOL)) {
			...
			if (!StringUtils.isBlank(password) && password.equals(old_password)) {
	...
	public Boolean AddNewUserAgent(String username, String nickname, String password, String nameagent, String address,
		...
		try (Connection conn = ConnectionPool.getInstance().getConnection("mysqlpool_admin");) {
			...
			if (createtime == null || createtime.trim().isEmpty()) {
				date1 = Date.from(LocalDateTime.now().atZone(ZoneId.systemDefault())
					    	      .toInstant());
			} else {
				date1 = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(createtime);
			}
			if (updatetime == null || updatetime.trim().isEmpty()) {
				date2 = Date.from(LocalDateTime.now().atZone(ZoneId.systemDefault())
			    	      .toInstant());
			} else {
			...
			stm.setString(index++, new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(date1));
			stm.setString(index++, new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(date2));
			if (site != null) {
...
//			ControlContrain(1);
	...
	public Boolean UpdateUserAgent(Integer id, String username, String nickname, String password, String nameagent,
		...
		try (Connection conn = ConnectionPool.getInstance().getConnection("mysqlpool_admin")) {
			PreparedStatement stm = conn.prepareStatement(sql);
			int index = 1;
			Date currentdate = Date.from(LocalDateTime.now().atZone(ZoneId.systemDefault())
		    	      .toInstant());
			stm.setString(index++, new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(currentdate));
			if (nickname != null) {
				stm.setString(index++, nickname);
			}
			if (nameagent != null) {
	...
	public Boolean UpdateUserAgent(UserAgentModel userAgentModel) throws SQLException {
		String sql = "UPDATE vinplay_admin.useragent SET nickname=?,nameagent=?,address=?,phone=?,email=?,"
				+ " facebook=?,`key`=?,`status`=?,namebank=?,nameaccount=?,numberaccount=?,`show`=?,`active`=?,"
				+ " updatetime=? where id=?";
		try (Connection conn = ConnectionPool.getInstance().getConnection("mysqlpool_admin")) {
			PreparedStatement stm = conn.prepareStatement(sql);
			int index = 1;
...
//        HashMap<String, Serializable> conditions = new HashMap<String, Serializable>();
	...
	public Map<String, Object> getAllChilds(Integer id, int pageIndex, int limit) throws SQLException {
		...
		try (Connection conn = ConnectionPool.getInstance().getConnection("mysqlpool_admin")) {
			...
			map.put("agents", userAgentModels);
	...
	public Map<String, Object> searchChilds(Integer id, String keyword, int level, int pageIndex, int limit) throws SQLException {
		...
		try (Connection conn = ConnectionPool.getInstance().getConnection("mysqlpool_admin")) {
			...
			String sql = "select id,username,nickname,nameagent,address,phone,email,facebook,`key`,`status`, "
					+ "parentid,`show`,`active`,`order`,sms,site,`level`,`code`,b.banks from useragent "
					+ "LEFT JOIN ( "
					+ "SELECT JSON_ARRAYAGG(JSON_OBJECT('agent_code', agent_code, 'bank_acount', bank_acount, "
					+ "'bank_code', bank_code, 'bank_name', bank_name, 'bank_number', bank_number, 'bank_branch', bank_branch)) banks, "
					+ "agent_code COLLATE utf8_general_ci AS agent_code "
					+ "from agentbanks "
					+ "GROUP BY agent_code "
					+ ") as b on b.agent_code=useragent.`code` "
					+ "where (`show` = 1 and `active` = 1)"
					+ parentIdCondition
					+ keywordCondition
					+ levelCondition
			...
			List<HashMap<String, Object>> userAgentModels= new ArrayList<HashMap<String,Object>>();
			ResultSet rs = stmt.executeQuery();
			while (rs.next()) {
				...
				item.put("nameagent", rs.getString("nameagent"));
				item.put("address", rs.getString("address"));
				item.put("phone", rs.getString("phone"));
				item.put("email", rs.getString("email"));
				item.put("facebook", rs.getString("facebook"));
				item.put("key", rs.getString("key"));
				item.put("status", rs.getString("status"));
				item.put("parentid", rs.getInt("parentid"));
				item.put("show", rs.getInt("show"));
				item.put("active", rs.getInt("active"));
				item.put("order", rs.getInt("order"));
				item.put("sms", rs.getInt("sms"));
				item.put("site", rs.getString("site"));
				item.put("level", rs.getInt("level"));
				item.put("code", rs.getString("code"));
				item.put("rank", 1);
				item.put("banks", rs.getObject("banks"));
				userAgentModels.add(item);
			}
	...
	@Override
	public Map<String, Object> searchChilds4AdminAgent(Integer id, String keyword, int level, int pageIndex, int limit) throws SQLException {
		Map<String, Object> map = new HashMap<>();
		try (Connection conn = ConnectionPool.getInstance().getConnection("mysqlpool_admin")) {
			pageIndex = pageIndex < 1 ? 0 : pageIndex - 1;
			String paginateCondition = (pageIndex == -1 || limit == -1) ? "" : (" limit ?,?");
	...
	@Override
	public Map<String, Object> searchChilds4BO(Integer id, String keyword, int level, int pageIndex, int limit) throws SQLException {
		Map<String, Object> map = new HashMap<>();
		try (Connection conn = ConnectionPool.getInstance().getConnection("mysqlpool_admin")) {
			pageIndex = pageIndex < 1 ? 0 : pageIndex - 1;
			String paginateCondition = (pageIndex == -1 || limit == -1) ? "" : (" limit ?,?");
	...
	@Override
	public Map<String, Object> searchParents(Integer id, String keyword, int level, int pageIndex, int limit) throws SQLException {
		Map<String, Object> map = new HashMap<>();
		try (Connection conn = ConnectionPool.getInstance().getConnection("mysqlpool_admin")) {
			pageIndex = pageIndex < 1 ? 0 : pageIndex - 1;
			String paginateCondition = (pageIndex == -1 || limit == -1) ? "" : (" limit ?,?");
			...
			String parentIdCondition = "";
			if((-1 != id))
				parentIdCondition = " and (find_in_set(id, (select ancestors from useragent where id = ?)) > 0 or id = ?)";
	...
	@Override
	public Map<String, Object> searchAgentMix(Integer id, String keyword, int level, int pageIndex, int limit) throws SQLException {
		Map<String, Object> map = new HashMap<>();
		try (Connection conn = ConnectionPool.getInstance().getConnection("mysqlpool_admin")) {
			pageIndex = pageIndex < 1 ? 0 : pageIndex - 1;
			String paginateCondition = (pageIndex == -1 || limit == -1) ? "" : (" limit ?,?");
			...
			String sql = "select id,username,nickname,nameagent,address,phone,email,facebook,`key`,`status`, "
					+ "parentid,`show`,`active`,`order`,sms,site,`level`,`code`, "
					+ "CASE "
					+ "WHEN id = " + id + " THEN 3 "
					+ "WHEN find_in_set(id, (select ancestors from useragent where id = " + id + ")) > 0 THEN 2 "
					+ "WHEN find_in_set(" + id + ",ancestors) > 0 THEN 1 "
					+ "ELSE -1 "
					+ "END AS `rank`, "
					+ "b.banks from useragent "
					+ "LEFT JOIN ( "
					+ "SELECT JSON_ARRAYAGG(JSON_OBJECT('agent_code', agent_code, 'bank_acount', bank_acount, "
					+ "'bank_code', bank_code, 'bank_name', bank_name, 'bank_number', bank_number, 'bank_branch', bank_branch)) banks, "
					+ "agent_code COLLATE utf8_general_ci AS agent_code "
					...
					+ "where (`show` = 1 and `active` = 1)"
					+ keywordCondition
					+ " ORDER BY `rank` desc, level asc, id desc" + paginateCondition;
			String sqlCount = "select count(id) total from useragent where (1=1)"
		  			+ keywordCondition;
			PreparedStatement stmt = conn.prepareStatement(sql);
			PreparedStatement stmtCount = conn.prepareStatement(sqlCount);
