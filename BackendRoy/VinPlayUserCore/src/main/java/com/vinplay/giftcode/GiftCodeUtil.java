		...
		try {
			if (!userService.isXacThucSDT(nickName)) {
				...
				if(user.getFacebookId()==null || "".equals(user.getFacebookId())) {
					return new BaseResponse<String>(Constant.ERROR_VERIFY_FB, "Please login via facebook or verifyphone to receive giftcode");
				}
			}
		} catch (Exception e2) {
...
//		}
    ...
    public static void insertGiftCode(GiftCodeModel giftCodeModel) throws SQLException {
    	String sql = "INSERT INTO gift_codes (giftcode,`type`,money,time_used,max_use,`from`,exprired,created_by,event,user_name)" +
                " VALUES (?,?,?,?,?,?,?,?,?,?)";
    	//TODO
    	if(giftCodeModel.type==3) {
    		giftCodeModel.max_use =100;
    	}
        try (Connection conn = ConnectionPool.getInstance().getConnection("mysqlpoolname")) {
            PreparedStatement stm = conn.prepareStatement(sql);
            int param = 1;
